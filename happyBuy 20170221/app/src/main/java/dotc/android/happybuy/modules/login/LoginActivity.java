 ...
 */
public class LoginActivity extends AbstractLoginActivity implements View.OnClickListener,RegistManager.RegistStatusCallBack{
...
//    private boolean mGetVerificationClickble=true;//验证码请求是否完成
    ...
    void initViews() {
        ...
        if (isLogin()&&HttpProtocol.UserType.ANONYMOUS.equals(type)) {
            mLoginForTourist.setVisibility(View.GONE);
        }


       /* findViewById(R.id.tv_login_for_phonr_tips).setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {

                String url = ConfigManager.get(LoginActivity.this).getH5Config().agreement;
    ...
    private void phone(String phoneNumber, String password){
        if(isFinishing()){
            return;
        }
        showLoginingDialog();
        AccountHelper.getInstance(getApplicationContext()).loginPhone(phoneNumber, password,listner);
    }
    ...
    AccountHelper.OnLoginResult listner = new AccountHelper.OnLoginResult() {
        ...
        public void onLoginSucceed(int type) {
            switch (type){
                case 1:
                    Analytics.sendUIEvent(AnalyticsEvents.VisitorLogin.Login_visitor_success, null, null);
                    break;
                case 2:
                    break;
                case 3:
                    break;
                case 10:
                    Analytics.sendUIEvent(AnalyticsEvents.PhoneLogin.Login_mobile_success, null, null);
                break;
            }
        ...
        public void onLoginFailed(int type,String message,int errorcode) {
            switch (type){
                case 1:
                    Analytics.sendUIEvent(AnalyticsEvents.VisitorLogin.Login_visitor_fail, message, (long) errorcode);
                break;
                case 2:
                break;
                case 3:
                break;
                case 10:
                    Analytics.sendUIEvent(AnalyticsEvents.PhoneLogin.Login_mobile_fail, message,  (long) errorcode);
                break;
            }
            dismissLoginingDialog();
            if(HttpProtocol.CODE.CODE_PWD_ERROR==errorcode){
                    ToastUtils.showShortToast(getApplicationContext(), getString(R.string.invite_code_enter_wrong));
            }else {
...
//        showLoginingDialog();
        Analytics.sendUIEvent(AnalyticsEvents.Login.Login_FB, null, null);
        mFbLoginHelper.login(new FbLoginHelper.LoginCallBack() {
            ...
            public void onLoginSuccess(FacebookSignInAccount account) {
                ...
                Analytics.sendUIEvent(AnalyticsEvents.Login.Login_FB_success, null, null);
                if(AppUtil.isActivityDestroyed(LoginActivity.this)){
                    return;
                }
                AccountHelper.getInstance(getApplicationContext()).loginWithFacebok(account,listner);
            }

            @Override
            public void onLoginFailed(int code, String message, Exception e) {
                Analytics.sendUIEvent(AnalyticsEvents.Login.Login_FB_fail, message, (long) code);
            ...
            public void onLoginCancel() {
                Analytics.sendUIEvent(AnalyticsEvents.Login.Login_FB_fail, "login cancel", -1l);
    ...
    private void onGoogleLoginClick(View view){
        showLoginingDialog();
        Analytics.sendUIEvent(AnalyticsEvents.Login.Login_G, null, null);
        mGoogleLoginHelper.login(new GoogleLoginHelper.LoginCallBack() {
            @Override
            public void onLoginSuccess(GoogleSignInAccount account) {
                HBLog.d(TAG,"onLoginSuccess "+Thread.currentThread().getName());
                Analytics.sendUIEvent(AnalyticsEvents.Login.Login_G_success, null, null);
                if(AppUtil.isActivityDestroyed(LoginActivity.this)){
                    return;
                }
                AccountHelper.getInstance(getApplicationContext()).loginWithGoogle(account,listner);
            }

            @Override
            public void onLoginFailed(int code, String message, Exception e) {
                Analytics.sendUIEvent(AnalyticsEvents.Login.Login_G_fail, message, (long) code);
                HBLog.d(TAG,"onLoginFailed "+Thread.currentThread().getName());
                dismissLoginingDialog();
            ...
            @Override
            public void onLoginCancel() {
                Analytics.sendUIEvent(AnalyticsEvents.Login.Login_G_fail, "login cancel", -1l);
                HBLog.d(TAG,"onLoginCancel "+Thread.currentThread().getName());
                dismissLoginingDialog();
            }
        });
    ...
    public void onClick(View view) {
        switch (view.getId()) {
            case R.id.img_facebook:
                Analytics.sendUIEvent(AnalyticsEvents.Login.Login_FB, "login", null);
                onFacebookLoginClick(view);
                break;
            case R.id.img_googleplus:
                Analytics.sendUIEvent(AnalyticsEvents.Login.Login_G, "login", null);
...
//                if(mVerificationBottonClickable&&mTimeFinished){
...
//                }
            ...
            case R.id.tv_login_for_tourist: {
                Analytics.sendUIEvent(AnalyticsEvents.VisitorLogin.Login_visitor, null, null);
                loginTourist();
            ...
            case R.id.login_for_phone: {
                ...
                if(!StringUtil.isEmpty(number)){
                    if(!StringUtil.isEmpty(verification)){
                        Analytics.sendUIEvent(AnalyticsEvents.PhoneLogin.Login_mobile, "login", null);
                        loginWithPhone(FormatVerifyUtil.checkMobileNumber(this,number),verification);
                    }else {
    ...
    private void loginTourist(){
        String anonymousUid= PrefUtils.getString(PrefConstants.ANONYMOUS.ANONYMOUS_UID, "");
        String anonymousToken= URLDecoder.decode(PrefUtils.getString(PrefConstants.ANONYMOUS.ANONYMOUS_TOKEN, ""));
        if(!StringUtil.isEmpty(anonymousUid)&&!StringUtil.isEmpty(anonymousToken)){
            anonymousLogin(anonymousUid,anonymousToken);
        }else {
            anonymousRegister(AppUtil.getDeviceId(this),"");
        }
    }
    private  void loginWithPhone(String number,String verification){
...
//                getVerification.setTextColor(getResources().getColor(R.color.verification_red));
            ...
            public void onFailed(int code, String message, Exception e) {
                ...
                if(HttpProtocol.CODE.VERIFICATION_COUNT_LIMIT==code){
                    Toast.makeText(GlobalContext.get(), R.string.vitification_tries_limit, Toast.LENGTH_SHORT).show();
                }else {
                    Toast.makeText(GlobalContext.get(), R.string.net_request_error, Toast.LENGTH_LONG).show();
                }
                mTimeFinished=true;
                if(mVerificationBottonClickable){
                    getVerification.setClickable(true);
